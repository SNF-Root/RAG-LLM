...
def parse_date_from_text(text: str) -> Optional[str]:
    ...
    # Remove label, ordinal suffixes, and normalize whitespace
...
def extract_prom_from_docling(file_path: str, debug: bool = False) -> Optional[PromForm]:
    ...
    # Look for date after "Date:" item
    if not date:
        for i, item in enumerate(texts):
            text = item.get('text', '').replace('\t', ' ').strip()
            if text == 'Date:' and i + 1 < len(texts):
                next_text = texts[i + 1].get('text', '').replace('\t', ' ').strip()
...
def extract_prom_from_docx(file_path: str, debug: bool = False) -> Optional[PromForm]:
    ...
    for i, text in enumerate(text_items):
        ...
        if 'requestor:' in text.lower():
            ...
            if 'date:' in text.lower():
                ...
                req_match = re.search(r'Requestor:\s*(.+?)\s+Date:', text, re.IGNORECASE)
                if req_match:
                    requestor = req_match.group(1).strip()
                ...
                if req_match:
                    ...
                    if not extracted or len(extracted) < 2:
                        # Look at next line
                        next_text = get_next_text(i)
                        if next_text and 'date:' not in next_text.lower():
                            requestor = next_text
                    else:
                        requestor = extracted
                else:
        ...
        if not date and 'date:' in text.lower() and 'requestor:' not in text.lower():
            parsed_date = parse_date_from_text(text)
            if parsed_date:
                date = parsed_date
            else:
    ...
    for i, text in enumerate(text_items):
        ...
        for label, field_num in SECTION_LABELS.items():
            ...
            if text_cleaned == label:
                is_match = True
            # Starts with label followed by colon, period, or space
            elif text_lower.startswith(label + ':') or text_lower.startswith(label + '.') or text_lower.startswith(label + ' '):
                is_match = True
            # Short text that equals label with common suffixes
            elif len(text_cleaned) < len(label) + 15 and text_cleaned.startswith(label):
                is_match = True
    ...
    for idx, (start_idx, field_num, label) in enumerate(section_starts):
        ...
        # Gather all text items between start and end
        content_parts = []
        for j in range(start_idx + 1, end_idx):  # Skip the label itself
            text = text_items[j]
            if text and len(text) > 1:  # Skip empty and single-char items (superscripts)
                ...
                is_label = any(lbl in text_lower or text_lower == lbl for lbl in SECTION_LABELS.keys())
                if not is_label:
...
async def embed_pipeline(prom_form: PromForm, embed_sem: asyncio.Semaphore) -> PromForm:
    ...
    if has_empty:
        return f"{', '.join(has_empty)} is returning None"
    ...
    with Pool(processes = (cpu_count - 2)) as pool:
        ...
        for result, bad in pool.imap_unordered(process_file, pdf_files):
            if bad:
                ...
                processed_files += 1
                print(f"Finished processing {result.request_title}")
