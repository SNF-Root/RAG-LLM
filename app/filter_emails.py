...
# mbox envelope line: "From user@... Thu Feb 14 15:30:16 2019"
...
# Reply introducer patterns: "On Thu, Feb ... wrote:" and variations
...
# Multi-line "On ... wrote:" - first line starts with "On" + date/time pattern
ON_DATE_MULTILINE_RE = re.compile(
    r"^On\s+(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun),?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)",
    re.IGNORECASE
)

# Generic quote header detection (language-agnostic):
# Match lines that have: date-like numbers + time (HH:MM) + angle brackets (<...>)
# This catches: "Op do 17 jan. 2019 om 13:50 schreef Name <email>:"
...
# Message-ID style strings: <...@...> (standalone)
MESSAGE_ID_LINE_RE = re.compile(r"^<[^@>]+@[^>]+>\s*$")

# Folded header continuation (line starts with whitespace, follows a header)
FOLDED_HEADER_RE = re.compile(r"^[\t ]+\S")

# Divider lines made of underscores/dashes (5 or more)
DIVIDER_RE = re.compile(r"^[_=-]{5,}\s*$")

# Attachment/part markers
ATTACHMENT_RE = re.compile(
    ...
    r"^Name:\s*\S+\.\S+\s*$|"  # Any filename with extension
    r"^Type:\s*(application|image|text|audio|video)/|"  # Any MIME type
    r"^Size:\s*\d+\s*bytes|"
    r"^Desc:\s*|"
    r"^URL:\s*<https?://)"
)

# Mailing list boilerplate keywords
...
# Outlook-style inline reply headers (bold markers with asterisks)
OUTLOOK_HEADER_RE = re.compile(
    r"^\*?(From|Sent|To|Cc|Subject|Date):\*?\s*.+$",
    re.IGNORECASE
)

# Outlook "On Behalf Of" style line
...
def extract_main_message(raw_email: str) -> str:
    ...
    for idx, line in enumerate(lines):
        ...
        # If in signature mode, skip short lines that look like signature content
        if in_signature:
            # Reset if we see substantial content (likely new section)
            if len(cleaned_line) > 80:
                in_signature = False
            else:
                continue
        
        # Reset consecutive filtered counter when we keep a line
        consecutive_filtered = 0
        ...
        if cleaned_line != "":
            ...
            # Keep single blank lines once body started
